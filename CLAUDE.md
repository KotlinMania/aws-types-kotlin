# Claude Code Project Instructions — aws-types-kotlin

## Project Overview

This is **aws-types-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`aws-types`](https://crates.io/crates/aws-types).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/aws-types/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/aws-types>
